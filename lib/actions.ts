...
export async function onSubmitAction(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  if (!parsed.success) {
    ...
    for (const key of Object.keys(data)) {
        if (key !== "products") {
            fields[key] = data[key as keyof Omit<FormDataType, "products">] as string;
        } else {
